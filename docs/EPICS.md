...
9.4. **[done 2026-08-19 — rescoped as built: FAILURE-ONLY notifications for snapshot-schedule runs (17.4) and replication runs (5.5), riding 16.12's pve-notify machinery; unit-proven (29 new tests, both emission matrices through the real run jobs); LIVE PROOF PENDING on a real node — one failed fire through the operator's own notification target]** As a user, I want **unattended** failures (a replication run or a snapshot-schedule fire that FAILED) to generate Proxmox notifications, so that I'm alerted through whatever channels I've already configured (email, Gotify, etc.). *(The built-ins evaluation this story waited on is **RESOLVED by 16.12's precedent**: PVE's notification system is the delivery layer (targets/matchers) but it emits nothing for a failing `anas-repl-*` / `anas-snap-*` unit, and ZED speaks only for ZFS pool events — so the gap was real, and the wiring 16.12 built for backup runs is exactly the wiring these two families needed. **Failure-only, and deliberately no per-task knob**: backup's `notify: always` default exists for vzdump parity and a body that replaced cron mail, but a snapshot schedule can fire hourly and `always` would be spam. A failed run notifies at severity `error`; success, skip, and everything else stays silent — the same policy the dashboard already states (16.7/17.7: failures surface, healthy is silent). No schema field, no UI; a knob can be added later if demand appears, and it would live where the policy does. Emission is at the **daemon's job each family converges on** — `POST /v1/schedules/:id/run` and the replicate job — so a timer fire and a UI Run Now both notify from one site (the timer runners stay thin clients, untouched), and it is **best-effort by contract**: a notification that cannot be delivered never fails or changes the job that emitted it. Bodies mirror the backup one — plain text, scannable, detailed enough to read instead of the run: schedule name+id / target (ZFS dataset or AHR pool), cadence + retention, or for a replication the `source → target` route including the peer/remote it was going to and the snapshot the run had settled on — with the **error text verbatim** in both. Nothing secret can ride a body. **OUT, explicitly: overdue detection** — an overdue task is precisely the one with no run to notify from, so building a checker loop would violate Principle 7 for a fact the UI already shows; **overdue stays PULL, in the dashboard warning cards**, exactly as 16.12 ruled for backups.)* — **Built:** `services/unattended-notify.ts` (the shared vocabulary the two families decide identically — outcome, the failure-only gate, severity, elapsed formatting — so they cannot drift; `formatElapsed` moved here and re-exported from `backup-notify.ts`, one copy); `services/snapshot-notify.ts` and `services/replication-notify.ts` (title/body/emission per family, where the content genuinely differs); the two run jobs' emission on completion AND in their catch; new `packaging/templates/anas-{snapshot,replication}-{subject,body}.txt.hbs` shipped, installed and removed alongside the AHR/backup pairs (the one `NOTIFY_TEMPLATES` list, uninstall.sh's matching names, make-release.sh's `anas-*.txt.hbs` set), with `type=anas-snapshot` / `type=anas-replication` so an operator can write matcher rules per event kind.
...
10. **Epic 9** — Jobs UI *(descoped 2026-07-18 — dedicated menu OBE, feedback is feature-native + dashboard, audit is journald; the one live slice, unattended-failure notifications (9.4/9.5), shipped 2026-08-19 on 16.12's pve-notify machinery — see epic status note)*
