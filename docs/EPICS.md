...
> - **Resource limits — `LimitNOFILE=1024` is the DEFAULT on generated units, per-task overridable.** Operator ground truth (pve10): without the cap, pbc **runs out of file handles because it keeps them open too long — worst in metadata change-detection mode**, suspected interaction with the network stack. The cap is backpressure: forcing early handle release keeps behavior sustainable (1024 is the classic soft limit pbc historically ran under). Expressed as `LimitNOFILE=` in the systemd unit (leverage, no prlimit wrapper); 16.1 verifies empirically on a large tree in both modes.
