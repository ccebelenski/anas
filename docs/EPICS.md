...
| Epic | What | Landed | History |
|---|---|---|---|
| 0 / 0.5 | Foundation (gateway + daemon, shared Zod, jobs, executor, journald audit) · stunt-node test infra + Playwright | V1 | §Epic 0, 0.5 |
| 1 | Auth — PVE owns the session; `PVEAuthCookie` verified locally (RSA-SHA1) | V1 | §Epic 1 |
| 2 | Dashboard — `GET /v1/status` aggregate, pool/share/disk/jobs/warnings, ZFS + AHR live telemetry | V1 · charts reworked 15.6 (0.2.10) | §Epic 2 |
| 3 | ZFS pools — parsers, observe, act; Disk Health; composer (3.23), special/dedup vdevs, PVE-pool tagging (3.25), root dataset first-class (3.26), busy-diagnosis (3.29), waste gate (3.30), RAIDZ expansion (3.31) | V1 → 0.2.0 | §Epic 3 |
| 4 / 4.5 | Datasets — tree, properties, POSIX-ACL permissions editor (4.7.2), compression/dedup/trim/sync toggles | V1 | §Epic 4, 4.5 |
| 5 | Snapshots — list/create/rename/rollback/destroy/clone | V1 | §Epic 5 |
| 5.5 | Replication — local + remote `zfs send/recv`, Replication menu, cluster remotes registry | V1 | §Epic 5.5 |
| 6 / 7 | SMB shares + NFS exports — round-trip parsers, surgical config editing, reload-as-side-effect, connect-string Details | V1 | §Epic 6, 7 |
| 8 | Share users/groups — `getent`-backed identities, nologin share users, SMB passwords | V1 | §Epic 8 |
| 9 | Jobs — menu descoped; **unattended-run notifications (9.4)** shipped on the pve-notify client | 0.2.10 | §Epic 9 |
| 10 | Setup & packaging — release tarball + transactional `install.sh`, semver single source, Actions release on tag | V1 | §Epic 10 |
| 11 | **AHR (ANAS Hybrid RAID)** — md→LVM→btrfs mixed-size pools, online expansion, spares, re-add, btrfs snapshots, notifications, dashboard parity | 0.2.x headline | §Epic 11, `AHR-DESIGN.md`, `AHR-GROUND-TRUTH.md` |
| 12 | Multi-node — central instance OBE; **12.1 version-skew banner**, **12.2 API through `:8006`** shipped | V1 / 0.2.0 | §Epic 12, `PROXY-TRANSPORT-DESIGN.md` |
| 13 | PVE UI embedding — native ExtJS panels (Ceph model), gateway forwards per node, cache-busted bundle | V1 | §Epic 13 |
| 15 | `ANAS.gfx` visual language — SVG objects, composer, topology, enriched trees, telemetry charts | V1 → 0.2.10 | §Epic 15 |
| 16 | **File backup via PBS** — repositories (incl. PVE's PBS storages), tasks as systemd units, Backup menu, cadence + biweekly parity, retention/prune, notifications | 0.1.x → 0.2.10 | §Epic 16 |
| 17 | Scheduled snapshots & scrubs — uniform ZFS/AHR schedules, hold-safe retention, Snapshots + Scrubs menus, last-scrub verdict, run/stop | 0.2.x | §Epic 17, `SCHEDULES-DESIGN.md`, `SCHEDULES-GROUND-TRUTH.md` |
| 18 | Mounts — remote NFS/CIFS as a client, verb ladder, inventory with hands-off tagging | V1 → 0.2.12 | §Epic 18 |
...
One line each. These are decisions, not history; the rationale is in `EPICS-HISTORY.md` under the epic named. A change that conflicts with one of these stops and gets discussed first. (`PRINCIPLES.md` and `DESIGN.md` sit above this list.)
...
### ZFS — pools, datasets, snapshots (3/4/5)
...
### AHR — ANAS Hybrid RAID (11) — design: `AHR-DESIGN.md`
...
### Schedules — snapshots & scrubs (17) — design: `SCHEDULES-DESIGN.md`
...
> **Headline: Epic 19 (iSCSI block storage) + Epic 16 phase 2 (snapshot-consistent backups and restore).** Two independent chains: iSCSI's core needs nothing from Epic 16; only LUN backup/restore (16.16/16.19) joins them. Staged like 0.2.10 — dev builds deploy to the fleet as stories land; the release is cut when everything is live-proven. File restore (16.17/16.18) is the severable piece if the release drags.
>
...
>
> Attention notes marked **⚠** came from the 2026-08-25 design review; the five that bite hardest if missed: LUN serial persistence (19.2), target boot ordering (19.5), initiator reconnect during a block restore (16.19), replication's base-snapshot discovery vs the transient backup snapshot (16.15), the FUSE hang trap in archive browsing (16.18).

### Epic 16 — phase 2: snapshot-consistent backups, boundaries, restore
...
16.13. `[open]` As a dev, I want ground truth on a disposable PBS + the stunt node before code: **restore** into an existing tree (what `--overwrite`, `--overwrite-files/-symlinks/-hardlinks`, `--allow-existing-dirs`, `--ignore-*` really do), **`--pattern`** syntax and escaping against picked paths (directories, names containing `*`/`[`), **`mount`** on a PVE node (fuse deps, `/dev/fuse`, unmount, what a crashed daemon leaves behind), **`.img`** from a regular file and restore *to a block device* (writes into the node, or replaces the path?) plus size-mismatch behavior, **metadata change-detection continuity** when a task switches live → snapshot root (⚠ inodes are preserved, so the prior `.mpxar` should still match — prove it, or the first snapshot-mode run re-reads 10 TB), `snapdev=visible` + udev node timing for zvols, and **AHR nested subvolumes** under a ro snapshot — so every parser and verdict is built on reality. *(Already known, 2026-08-25: the client is `tar --one-file-system` — a child dataset is an EMPTY DIR in a live backup (`skipping mount point: …` logged) and is unreachable from the parent's `.zfs/snapshot` in any mode; plain subdirectories are fine.)*

##### Enhancement — filesystem boundaries are visible, never silent
16.14. `[open]` As a user, I want nested filesystems under a backup source **detected, shown, and chosen — never silently skipped**: the wizard and the task detail list every mountpoint nested under a source with its kind (child dataset, btrfs subvolume, local disk, NFS/CIFS mount, pmxcfs); a per-archive **`includeNested: none | all | [paths]`** (→ the client's default / `--all-file-systems` / one `--include-dev` per path); an alert when the current choice would back up a nested filesystem as an empty directory; and the client's `skipping mount point:` lines parsed into run warnings — so an omission is on the record, not in a log nobody reads. **Default = `none`** (the client's own; PVE's lead). *(⚠ Absent = `none`, and the edit dialog shows it as absent — never writes a default on an untouched save. ⚠ Detection via `findmnt` submounts must exclude `.zfs/snapshot` automounts and treat autofs placeholders as the mounts family does. Own detection is the authoritative warning; the log parse is secondary. Product-level example for the story text: backing up `/etc` today silently omits `/etc/pve`.)*

##### Snapshot-consistent backups
16.15. `[open]` As a user, I want ZFS- and AHR-backed sources backed up **from a snapshot** — snapshot → back up from the snapshot path → destroy — so a multi-hour run captures one instant, with sources that cannot be snapshotted (remote mounts, foreign filesystems) backed up live and **labeled live**. *(Consistency is derived per source from its capability, shown in the wizard and the run result.)* ⚠ **Reuse, don't re-implement:** `zfs snapshot` is already invoked in two places (`routes/datasets.ts`, `services/snapshot-schedules.ts`) — extract ONE helper first and make backup its third caller; AHR goes through `createAhrSnapshot`. ⚠ **Child datasets force the shape:** recursive snapshot → **one archive root per dataset, expanded by the runner at run time** from the single stored source (deterministic names, e.g. `data` + `data__photos`, within PBS's archive-name charset; the wizard says "N child datasets → N+1 archives"); a plain subdirectory source maps to `<dataset>/.zfs/snapshot/<s>/<relative>`; excludes apply per expanded root. The 16.14 matrix is explicit: children = expansion; non-snapshottable nested mounts = live-labeled or skipped, per the option. ⚠ **Lifecycle:** transient `anas-backup-<task>-<ts>` snapshots are destroyed in `finally`, a stale-prefix sweep runs at every start (recursive), **no holds**. ⚠ **Replication's newest-common-snapshot discovery and Epic 17 retention must ignore the `anas-backup-*` prefix** — otherwise a transient becomes an incremental base and its destroy breaks the chain. ⚠ Continuity proven per 16.13 before the toggle ships.

##### Block images
16.16. `[open]` As a user, I want a backup archive of kind **`img`** beside `pxar` — a block device or a raw image file backed up as a fixed-chunk image (the client's `.img` type) — so an iSCSI LUN (Epic 19), either backing kind, is a source in the same task wizard, on the same timers, with the same prune/notify/dashboard. *(Zvol: snapshot device via `snapdev=visible` set-use-restore + `udevadm settle`; image file: from the dataset/subvolume snapshot per 16.15; regular-file `.img` per 16.13's finding, else `losetup -r`. ⚠ A live-written image's mtime always moves, so metadata mode can never skip it: **the wizard states "full read every run"**; upload and storage are chunk-deduped. Backup of a live LUN is crash-consistent — say so.)*

##### Restore
16.17. `[open]` As a user, I want a **desktop-grade directory picker** (the 16.9 known issue) — PVE's expanding-tree idiom with breadcrumb + type-ahead over `GET /v1/fs/browse` — with **two backends: the live filesystem (backup wizard) and a mounted archive (restore)** — so picking a path or a file to restore works the way a desktop does. *(A selector, not a content browser — the content browser was rejected and this does not reopen it.)*

16.18. `[open]` As a user, I want to **restore files from a PBS backup** — pick a task (or, for archives whose task was renamed/deleted, a repository + group), a point in time (`snapshot list`, user-initiated), an archive, then **select anything from one file to the whole tree** with 16.17's picker over a FUSE `mount` of the archive; restore runs as a job via `restore --pattern …` (native extraction; FUSE only ever reads). **Target: a new directory beside the source by default; in-place is an explicit toggle** (→ `--overwrite*` + `--allow-existing-dirs`); ownership/ACL/xattr toggles surfaced honestly; a space check from the manifest first. *(⚠ **The FUSE hang trap:** reads go to PBS over the network — browse must be timeout-guarded like `mounts.ts`, the mount lives in a private `0700` dir under `/run/anas`, is unmounted when the job ends, and a stale-FUSE sweep runs at daemon start. ⚠ The 409 confirm gate applies only to an in-place restore of a TREE; a single explicitly picked file restored in place is a checkbox. ⚠ LUN backing paths are protected targets, like PVE territory. ⚠ Cancel leaves the side-by-side dir labeled partial or removes it — never a silent half. ⚠ An expanded archive whose child dataset no longer exists restores side-by-side with an explanation, never auto-creates datasets.)*

16.19. `[open]` As a user, I want to **restore a LUN image from a PBS backup** — whole image, by nature — onto its zvol or image file, so a block device comes back to a point in time. *(⚠ **The LUN must be disabled for the duration, not merely checked**: initiators (open-iscsi, Windows) auto-reconnect — disable the LUN/TPG, write the image back (file kind: `restore` to the path; zvol: into the device or `map` + `dd`, per 16.13), **preserve the unit serial** (19.2), re-enable. An image larger than the current target is refused with both sizes. Live-session 409 as the entry gate.)*

### Epic 19 — iSCSI block storage
...
##### Dev (ground truth first)
19.1. `[open]` As a dev, I want ground truth on the stunt node with `targetcli-fb` installed and at least two initiators (the node's own `open-iscsi`; a PVE `iscsi:` storage entry as the "PVE consumes it" proof; a Windows VM if one is available, for serial and CHAP quirks): `saveconfig.json` shape; configfs session listing; whether `targetcli` batches commands; **boot restore with a backing device missing** (⚠ 19.5); **how the unit serial/`wwn` is set and replayed** (⚠ 19.2); **`fileio` size on resize** (fixed at creation ⇒ recreate ⇒ serial replay); `block` capacity after a zvol grow; `emulate_tpu`/`tpws`, `block_size`, `write_back` vs `write_thru`; ACL + CHAP (incl. the 12–16-byte Windows secret rule) + mutual CHAP; portal binding incl. IPv6; **`zpool export` with a LUN open**; `/dev/zd*` and loop devices in `lsblk` vs the composer's candidate filter — captured as fixtures with real-vs-synthetic labels, written up as `docs/ISCSI-GROUND-TRUTH.md`.

19.2. `[open]` As a dev, I want the shared schemas and the read layer: `IscsiTarget` / `IscsiPortal` / `IscsiLun` (kind, backing, size, **serial**, mapped initiators) / `IscsiInitiator` / sessions, parsed from `saveconfig.json` + configfs; **ownership derived without shadow state** (backing path on an ANAS-managed pool/dataset + naming convention ⇒ ANAS; anything else ⇒ tagged hands-off, the 3.25 pattern; **PVE's `vm-NNN-disk-M` zvols are never candidates**); and `GET /v1/iscsi/*` (resource table added to DESIGN.md when this lands). ⚠ **Serial persistence is designed in here:** every path that (re)creates a backstore — boot restore, fileio resize, image restore — replays the stored serial explicitly; Windows and ESXi identify a LUN by it and treat a new one as a different disk.

##### Volumes
19.3. `[open]` As a user, I want **ZFS volumes first-class in Datasets**: listed with size / `volblocksize` / used, create (size, `volblocksize`, sparse), resize, snapshot/rollback/destroy through the existing verbs, "Share this" disabled (no path), PVE-owned zvols tagged hands-off — so the ZFS block object is manageable where the rest of the dataset tree is. *(Image files are created by the iSCSI screen on a chosen dataset or AHR pool, not here. ⚠ The Datasets dialog-contract harness must cover the volume type.)*

##### The iSCSI menu
19.4. `[open]` As a user, I want an **iSCSI** menu: targets (IQN, portals, LUN count, sessions, enabled) with per-target LUNs (kind, backing, size, serial, mapped initiators, connected-now) and initiator ACLs; **create target** (stable ANAS-generated IQN, editable once, immutable after — a "rename" is a new target; portal address picked from PVE's network config; explicit ACLs; CHAP optional, mutual optional, secrets write-only); **add LUN** (pick an existing zvol, or create a sparse raw image file of a given size on a dataset / AHR pool); enable/disable; delete — every mutation a job. *(⚠ `targetcli` is not transactional: LIO mutations are serialized behind a daemon mutex, `saveconfig` is copied before every mutation (rotating, in ANAS's own dir) and each change is a journald audit line — recovery artefacts, never shadow state. ⚠ Backstore names are flat: `pool/vol` needs a collision-free encoding. ⚠ Sensible attribute defaults: `emulate_tpu`/`tpws` on so UNMAP reclaims thin space; `write_thru` for fileio with `write_back` behind a data-loss warning; `block_size` and `volblocksize` guidance. ⚠ CHAP secret length validated in the schema.)*

##### Boot, lifecycle, gates
19.5. `[open]` As a user, I want targets to **survive reboot correctly**: `rtslib-fb-targetctl` enabled with an **ordering drop-in after `zfs-import` / AHR activation** and the reverse at shutdown (target down before pools export), modules loaded on first use only, and a LUN whose backing device is missing at boot surfaced as a dashboard warning (new `iscsi` category) instead of vanishing silently. *(⚠ The A1 review finding; seam = `zfs-import-unit.ts`.)*

19.6. `[open]` As a user, I want the rest of ANAS to **know a LUN is there**: destroying, rolling back, exporting or unmounting a dataset, zvol, pool or AHR pool under a LUN is refused with "held by LUN <target>/<n>" (the 18.5 cross-check pattern; `busy-diagnosis.ts` names LIO); a zvol snapshot rollback under a live session is refused outright; deleting/resizing a LUN with a live session is a 409; a portal whose address is gone shows in status; a PVE firewall that would block 3260 is warned about, never edited; the disk inventory excludes `/dev/zd*` and loop devices from composer candidates.

##### Live-proof
19.7. `[open]` As a dev, I want the whole arc live-proven on the stunt node through the real API: create target + zvol LUN + file LUN → connect from `open-iscsi` and from a PVE `iscsi:` storage → ACL rejection of an unlisted initiator → CHAP → write data → reboot the node, data and serial intact → resize both kinds with the serial preserved → busy refusals (destroy under LUN, delete with a session) → back up both LUNs to the disposable PBS (16.16) and restore one (16.19) → delete target, `saveconfig.json` copies and journald audit present — before any release gate.
...
- **Local-snapshot file restore** *(decoupled from Epic 16 phase 2, 2026-08-25)*: the 16.17 picker over `.zfs/snapshot/<s>/` and AHR ro snapshots, copying selected files back without a rollback. No backup client involved — "just files". Its own small epic when wanted.
...
- **Epic 14 — directory services (AD/LDAP via realmd/winbind/sssd) + NFSv4 ACLs (4.7.1)**: enterprise; deferred until demand. The `getent` seam keeps it a drop-in.
...
| qcow2-backed LUNs | Unique but low value; needs a daemon per LUN; tiers 1+2 suffice | 19 (2026-08-25) |
| LV-backed LUNs on AHR | Outside AHR's btrfs data model; file-on-btrfs is the AHR block object | 19 (2026-08-25) |
| PVE "ZFS over iSCSI backend" wizard / `pvesm` snippets | ANAS is the target side only; PVE is an ordinary initiator | 19 (2026-08-25) |
