# ANAS — Epics & Stories: HISTORY (frozen 2026-08-25)

> **This file is the archived ledger.** It is the complete `docs/EPICS.md` as it
> stood at the 0.2.12 → 0.3.0 boundary, preserved verbatim (only this header was
> added). Every shipped story's design notes, live-proof accounts, incident
> write-ups, and dates live here. **Nothing in it is deleted or rewritten.**
>
> **The live plan is `docs/EPICS.md`** — active work, standing rulings, serious
> candidates, and the rejected/OBE list. Read THIS file only when you need the
> *rationale* behind a ruling, or the build history of a shipped epic.
>
> Rulings recorded here remain binding unless `docs/EPICS.md` explicitly
> supersedes them; where the two disagree, `docs/EPICS.md` wins.
...
9.4. **[done 2026-08-19 — shipped failure-only, then extended THE SAME DAY to FULL backup parity on an operator ruling ("we should notify on successes too", and "probably doesn't default to 'on'"): a per-task notify MODE for snapshot-schedule runs (17.4) and replication runs (5.5), riding 16.12's pve-notify machinery; unit-proven (59 tests across the two rounds — both mode gates x all four outcomes, both stores' round-trip, both hook sites); LIVE PROOF PENDING on a real node — one failed fire and one `always` success through the operator's own notification target]** As a user, I want **unattended** run outcomes (a replication run or a snapshot-schedule fire) to generate Proxmox notifications, so that I'm alerted through whatever channels I've already configured (email, Gotify, etc.). *(The built-ins evaluation this story waited on is **RESOLVED by 16.12's precedent**: PVE's notification system is the delivery layer (targets/matchers) but it emits nothing for a failing `anas-repl-*` / `anas-snap-*` unit, and ZED speaks only for ZFS pool events — so the gap was real, and the wiring 16.12 built for backup runs is exactly the wiring these two families needed. **Parallel construction, settled 2026-08-19**: the first cut shipped failure-only with no knob; the operator invoked the like-things-work-alike rule, so these two families now work like backup IN FULL — the same `notify: 'always' | 'on-failure'` field, the same four outcomes (success / warning / failure / skip), the same gate, the same combo in the dialog. **What deliberately DIFFERS is the DEFAULT, and only the default**: backup keeps `always` (vzdump parity — it replaced cron mail that sent every run), while snapshot schedules and replication default to **`on-failure`** — a schedule can fire every 15 minutes, so success mail is spam nobody reads, and the quiet default makes the upgrade behaviour-preserving: a schedule or task stored before the field parses back as `on-failure`, exactly the failure-only behaviour the morning's build shipped. `always` is the explicit opt-in. **Outcomes**: `always` mails success + warning + failure; `on-failure` mails warning + failure (a completed-but-imperfect run is what an on-failure operator asked to hear about); a skip mails in neither. A snapshot fire whose prune had to SKIP HELD snapshots (17.6's `skippedHeld`, GT-7) is a **warning** — the snapshot was taken but retention could not do what policy asked; a replication whose `zfs hold`/`release` fail-open hiccups produced warnings is likewise a warning, since an unpinned base is a future broken incremental chain. **Where the mode lives**: a schedule's rides its unit JSON (`X-ANAS-Schedule`), read straight off `schedule.notify` in the run job; a replication task's rides its unit JSON (`X-ANAS-Task`) but the ONE place that notifies is the one-shot replicate endpoint, which knows nothing about tasks — so the timer's runner carries it there as a `--notify always` ExecStart flag (emitted only for the opt-in, keeping a quiet task's ExecStart byte-identical) and forwards it in the request body, the one place a runner gained a flag and still a dumb conduit. Emission stays at the **daemon's job each family converges on** — `POST /v1/schedules/:id/run` and the replicate job — so a timer fire and a UI Run Now both notify from one site, and it is **best-effort by contract**: a notification that cannot be delivered never fails or changes the job that emitted it. Bodies mirror the backup one — plain text, scannable, detailed enough to read instead of the run: schedule name+id / target (ZFS dataset or AHR pool) / snapshot taken / what prune destroyed and held / cadence + retention; or for a replication the `source -> target` route including the peer/remote, the snapshot, the send mode and its incremental base — with the **error text verbatim** on a failure and the warnings verbatim on a warning. Nothing secret can ride a body. **OUT, explicitly: overdue detection** — an overdue task is precisely the one with no run to notify from, so building a checker loop would violate Principle 7 for a fact the UI already shows; **overdue stays PULL, in the dashboard warning cards**, exactly as 16.12 ruled for backups.)* — **Built:** `NotifyMode` in shared `schemas/common.ts` (ONE enum for all three families; `BackupNotifyMode` is now an alias of it) plus the `notify` field on `SnapshotSchedule`, `ReplicationTask` and `ReplicateRequest`, all additive-and-defaulted so old UI ↔ new daemon and new UI ↔ old daemon both degrade to the quiet mode rather than error; `services/unattended-notify.ts` promoted to the vocabulary ALL THREE families share (the four-outcome union, the ONE `shouldNotify(mode, outcome)` gate, severity, elapsed formatting — `backup-notify.ts` now delegates to it rather than keeping a second copy); `services/snapshot-notify.ts` and `services/replication-notify.ts` extended to titles/bodies for every outcome; `ANAS.notifyMode` in `00-core.js` (the normalizer + the combo + the detail row, used by the Backup, Snapshots and Replication dialogs alike — three copies of a two-value enum would have drifted), with the mode carried through create, edit AND enable/disable so a toggle cannot silently reset it. The `anas-snapshot` / `anas-replication` template pairs from the morning's build are unchanged — success and warning ride the same ones, exactly as backup does. — **Body fixes from live ground truth (same commit, 2026-08-19):** a real gotify delivery showed `Repository: pve:store:store / ns` (a PVE-sourced repo is NAMED `pve:<datastore>`, and `backupTargetLine` appended the datastore unconditionally) — the suffix is now appended only when the name does not already end in it, exact-match, so a repo named for a DIFFERENT datastore still spells it out. The same delivery mojibake'd an em-dash, so **every notification title and body is now pure ASCII** (the `source -> target` arrow included) with a guard test per family: we do not own the PVE→delivery pipeline that reads our UTF-8 as Latin-1, so we stop handing it anything to get wrong. And the editorial closing lines are **gone** from all three bodies (operator: "unnecessary commentary") — a body ends on the facts.

#### Dev
9.5. **[done — satisfied by 16.12 (2026-08-17), confirmed 2026-08-19]** As a dev, I want a Proxmox notification client that can send notifications through the PVE notification system, so that ANAS events reach users through their existing alert channels. *(The client exists: `packages/daemon/src/services/pve-notify.ts` — one `PVE::Notify::notify` invocation with severity/title/message passed as argv (never interpolated), a whitelist-validated template name selecting the shipped `.hbs` pair and the matcher `type`, and best-effort delivery by contract. Built for AHR (§7.2/GT-17), extended additively by 16.12, and reused unchanged by 9.4 — no second client was needed.)*
...
15.6. [done 2026-08-19] **Telemetry chart legibility** — `gfx.timeChart` and the pool telemetry blocks it feeds. *(Operator design review 2026-08-19, from live pool blocks: the charts were arithmetically correct and hard to read. Five rulings, all client-side — the telemetry API and daemon are untouched, every derived figure comes off sample rings already in hand. Because the chart is ONE shared component, ZFS and AHR come out identical with no per-view fork.* **(a) 1-2-5 ladder + ratchet.** *Scale maxima come off a fixed ladder — 1/2/5 × each power of ten within each BINARY unit family (… 500 KiB/s · 1 MiB/s · 2 · 5 · 10 · 20 · 50 · 100 · 200 · 500 MiB/s · 1 GiB/s …) — so a maximum is always a number with a name and the mid gridline (half a rung) is always clean; no more data-driven "4.77 MiB/s". The scale RATCHETS: up immediately on any sample that would clip, never down on its own — a held scale outlives its peak so "back to normal" can't be confused with "rescaled". The ratchet state is a caller-owned object living on the view, so gfx stays stateless and a closed panel resets it with its buffers. Sizing the left margin from the ladder's widest POSSIBLE label (not the current one) also stops the plot's left edge jittering as the scale moves — the stability half of 11.15's margin fix.* **(b) Operator-triggered re-fit.** *A quiet line-icon control at the chart's right edge, present ONLY while the held scale sits above the window's own fit rung ("elements earn their keep" — otherwise it is a permanent no-op). One click re-fits to the window and the ratchet resumes from there.* **(c) Single-vdev collapse.** *Where a pool has exactly ONE vdev (or ONE AHR band) that tier's chart and I/O/latency readouts are the pool block's numbers repeated verbatim; it keeps its header row — name, type, device count, state pill, the things the pool block does NOT say — and drops the duplicates. Two or more render in full, because then they can disagree. **Per-member disk tiles are never collapsed or reduced** — a failing disk diverges from its siblings at exactly that layer (operator nit, explicit). The AHR band's sync strip is likewise never collapsed: md recovery traffic is invisible in the pool's own I/O counters by definition.* **(d) Bursty-workload honesty.** *An instantaneous figure is a lie on a txg-cadence workload — a pool moving 241 MiB/s reads "0 r · 0 w IOPS" between flushes. Every headline slot now LEADS with a labelled ~10s rolling average over the existing ring, keeping peak/avg as they were. On the chart the raw per-sample line is kept exactly as-is — a txg stall is real behaviour and must stay visible — and a thin same-colour average overlay is added on top of it.* **(e) Absence looks like absence.** *The buffers start when a view opens, so the left of a chart is time never measured; blank canvas over a zero baseline is indistinguishable from a measured idle. That region is now hatched and labelled "no samples yet" (chosen over growing the window, which would have churned the X axis under a live series). And every number names its direction and its window — the unlabelled ▼/▲ glyphs are gone in favour of explicit `R`/`W`, and each figure carries `avg 10s` / `peak 5m` / `avg 5m`. Supporting fix: the rolling buffers now accept ONE sample per telemetry tick however many times a section re-renders, so a /status load, a manual Refresh or a re-fit click cannot bend peak, average or the chart's time axis. Verified by two committed headless harnesses under `packages/pve-integration/test/` — `gfx-timechart.harness.mjs` (the shared chart: ladder across magnitudes, ratchet hold after peak expiry, re-fit visibility, y-label width stability) and `dashboard-telemetry.harness.mjs` (the composite: solo collapse for ZFS and AHR alike, labelled figures, one-sample-per-tick, ratchet across real poll ticks).)*
...
10. **Epic 9** — Jobs UI *(descoped 2026-07-18 — dedicated menu OBE, feedback is feature-native + dashboard, audit is journald; the one live slice, unattended run notifications (9.4/9.5), shipped 2026-08-19 on 16.12's pve-notify machinery — see epic status note)*
...
> **Refinement (operator-requested 2026-08-19):** the two pool verbs that lived ONLY inside the pool-detail pop-up — **Snapshots…** (11.12) and **Change mount…** — are **promoted to the Hybrid RAID grid toolbar** as selection-dependent buttons beside Scrub, and removed from the pop-up. Actions on a pop-up are too hidden — the snapshot feature went unnoticed for exactly that reason — and the Pools view already settled the rule: *actions live on the grid toolbar; the detail window is display* (parallel construction). The detail window is now display-only (Reload/Close). The 11.12 flat-layout gate rides along unchanged, now driven from the grid selection: a `subvolLayout: false` pool keeps Snapshots disabled carrying the same "destroy and recreate — no in-place migration" advisory. Pure UI, one file; the grid store simply reads the `subvolLayout` GET /v1/ahr already returns.
...
##### Notifications (added 2026-08-17)
16.12. **[done 2026-08-17 — built and unit-proven (27 new tests: the full emission matrix through the real run job); LIVE PROOF PENDING on stunt — one real run's mail through the operator's own notification target]** As a user, I want a finished backup run to **tell me what happened, through PVE's own notification system** — with the detail the cron mail had — so a failed or a merely-warning run reaches me instead of waiting to be noticed in the dashboard. *(**Operator ruling, binding** (2026-08-17): *"Follow the PVE lead — I would absolutely want the more detailed notifications - on-completed not just on-failure."* So the shape is vzdump's, default and all: a per-task **`notify: always | on-failure`, DEFAULT `always`**, riding the X-ANAS-Task unit JSON like every other field (16.3 — units ARE the store; **absent = `always` via the schema default, so no task written before this story migrates**). Semantics, FINAL: emission is at the **daemon's run job**, the ONE point every real run reaches — a timer fire and a UI Run Now both arrive there through the task's own unit (16.5 Fix 1), so one site covers both. The matrix: **success → notify only in `always`, severity `info`**; **completed-with-warnings** (e.g. 16.11's prune that failed after a good backup) **→ notify in BOTH modes, severity `warning`**; **failure → notify in BOTH modes, severity `error`**, carrying the actual error text; **`skipped-off-week` (16.10) → NEVER notifies, in either mode** — the cadence gate produced no run and the biweekly cron jobs it replaces produced no mail, and a non-event is a non-event. (The benign too-soon collision is not a skip in this sense: pbc really ran, so it notifies as a success whose body says plainly that nothing needed backing up.) **The body is the whole point** — task, `repo:datastore / namespace`, backup-id, the per-archive stats lines pbc printed verbatim (`work.pxar: had to backup 419 GiB … (average 166 MiB/s)` / `reused 41 GiB (8.9%)`), duration (pbc's own `Duration:` when it printed one, else the job's wall clock, labeled), prune kept/removed/protected when retention ran, warnings verbatim, and the error on a failure. Plain text, scannable. **OUT, explicitly: an overdue PUSH.** No process exists to send it — an overdue task is precisely the one with no run to notify from — so building a poller to manufacture the event would violate Principle 7 for a fact the UI already shows; **overdue stays PULL, in the dashboard warning category**, and saying so is more honest than a silence that looks like coverage. No new deps, no poller, no PBS contact — the run is the event source.)* — **Built:** shared `BackupNotifyMode` + `BackupTask.notify` (defaulted); `services/pve-notify.ts` extended **additively** with a template/type parameter (one Perl invocation, not a second — the AHR callers are untouched and pinned by their existing tests, and the template name is the only interpolated value, whitelist-validated); `services/backup-notify.ts` (outcome mapping, the mode gate, severity, title and the body builder — `pruneSummaryLine` reused, never re-spelled); the run job's emission at completion AND in its catch, best-effort by contract (a notification failure never fails the run); new `packaging/templates/anas-backup-{subject,body}.txt.hbs` shipped, installed and removed alongside the AHR pair (install.sh now carries ONE `NOTIFY_TEMPLATES` list its preflight and its install step both read; uninstall.sh removes exactly those names; make-release.sh stages the whole `anas-*.txt.hbs` set), with `type=anas-backup` so an operator can write a matcher rule for backup events specifically; and the wizard's **Notification mode** combo (Always / On failure, read straight off the field by itemId — no hiddenfields, issue #26) with the mode carried through edits and enable/disable toggles and shown in the task detail.
...
18.5. [done 2026-07-18 — live-proven end-to-end on the stunt node: NFS+CIFS lifecycles, creds never in fstab/ps/journald, the full verb ladder INCLUDING a reboot (disabled stayed down, enabled remounted), busy-unmount 409 + confirm retry, all test verdicts, dead-server → unreachable in ~2s → dashboard mount warning → recovery, and /etc/fstab byte-identical to pre-ANAS baseline after full lifecycle. Live-proof caught 2 real defects, fixed: the executor collapsed async exit codes to 1 (err.status vs err.code — lost timeout's 124, latently daemon-wide) and the binfmt_misc autofs placeholder leaked into the inventory as a spurious 'armed' row. Design nit resolved 2026-08-19 — the leftover empty mountpoint dir is now a DIALOG CHOICE: Remove Mount carries an "Also remove the empty directory" checkbox, CHECKED by default (leftover dirs are clutter, so the box is the opt-out), which rides the DELETE as `?removeMountpointDir=true`; the flag defaults to false, so a client that predates it deletes exactly as before. The daemon removes the dir with **rmdir semantics only** — empty, never recursive, never on a still-mounted path — and a dir that stayed (not empty, still held) comes back as a warning on the COMPLETED job, surfaced in the view: a leftover dir never fails the removal the user asked for.] As a user, I want to mount remote NFS and CIFS shares as a client (the old-NAS migration path, and remote backup targets), with CIFS credentials in a root-only credentials file — never inline in fstab, never on argv — so remote data is reachable without hand-editing anything.
...
> **Grid legibility (operator screenshot review 2026-08-19 — 18.3's view, pure UI):** the ownership tags that rode inside other columns — the `LOCAL`/`PVE` badge stapled to Mountpoint, the `remote` pill stapled to Type — are consolidated into a column of their own, **Managed by** (PVE / AHR / ANAS / LOCAL, each keeping its badge, its colour and its hands-off tooltip); Mountpoint and Type are plain values again. **Persist** now names the persistence MECHANISM instead of merely "fstab or not": `fstab` → `zfs` (mounted on pool import from the mountpoint property) → `storage.cfg` (PVE mounts it) → `session`, in that precedence — so `session` is reserved for a genuinely unowned mount that nothing brings back at boot, rather than libelling every ZFS pool, the cephfs and every PVE storage mount as ephemeral. Every fact was already on the inventory row; no daemon or schema change.

> **Mounts-family contract fixes (2026-08-24 — issues #34, #35, #38 + the silent-default finding):** three defects that all came from the same place — the dialog and the daemon disagreeing about what a field MEANS.
> - **#34 — a blanked option could not be cleared.** The dialog sent an option only when it had a value, so a blanked field was indistinguishable from an untouched one and the daemon (rightly) kept the old value: set an idle timeout, an `rsize`, a `domain`, and it could never be removed again. Fixed as a CONTRACT, not per-field: `MountRequestOptions` gives every value-bearing option a `null` (= remove), the dialog sends every option on every save (value or null), and one shared `applyOption` helper — used by BOTH write paths, create and edit — makes value/null/omitted mean set/clear/keep everywhere. `extraOptions` follows the same rule (a blank field drops the passthrough), and unchecking Automount now takes `x-systemd.idle-timeout` with it (decided in the merge; the fstab renderer stays dumb). A CLASS GUARD walks every key of `MountRequestOptions` — set it, clear it, prove the token left the rendered line — so an option added later without clear semantics fails the suite.
> - **#43 item 3 — an untouched edit wrote options the entry never had.** The field configs carry defaults (`vers=4.2`/`3.1.1`, `file_mode=0644`, `dir_mode=0755`, idle-timeout 60); an edit left them showing and the unconditional save would have written all of them. The pre-fill now reflects the entry EXACTLY — no value in the entry means a blank field, blank means absent — so opening an entry and pressing Save rewrites the line byte-for-byte (regression test: parse → build the body from that entry's own pre-fill → the line is unchanged). `hard` stays honest as the one fstab tri-state (`hard` / `soft` / neither).
> - **#35 — an idled-out automount lost its remote identity.** Once the mount idled away, findmnt showed only the autofs placeholder (`systemd-1`, fstype `autofs`), which the inventory projected verbatim: the row came back as LOCAL and the UI disabled Edit / Unmount / Remove on a perfectly good CIFS share. An ARMED row now takes its identity (type, remote, source, server/share) from the fstab entry it is already correlated with; the placeholder only gets to say the STATE. A mounted automount is untouched — it was already right.
> - **#38 — Server and Export path were dead controls on an edit.** `UpdateMountRequest` carries neither (the spec is identity, like the mountpoint), so an edited value was stripped by Zod and silently dropped — while Test connection cheerfully validated it. Both fields are now read-only on an edit and Test probes the STORED spec, so the diagnosis can never be about a configuration the save will not apply.
