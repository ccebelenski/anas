...
| `POST` | `/v1/mounts/:mountpoint/state` | `{action: mount\|unmount}` — mount/unmount WITHOUT touching the fstab entry (temporarily offline a persisted mount); busy-unmount → 409 as above. Added at Stage-2/3 reconciliation 2026-07-18: DELETE removes the entry, so persisted mounts had no temporary-offline path. Pools action-subresource idiom. | `202`/`409` |
