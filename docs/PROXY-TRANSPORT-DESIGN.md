...
- **Node without ANAS (e.g. a Ceph-only node):** `:8006` is always up (it's PVE), so `/anas` **404s** (pveproxy "no such file") instead of `:3000`'s connection-refused. The gateway must detect pveproxy's no-such-path response and return a clean **`ANAS_NOT_INSTALLED`** for that node; the UI shows a "ANAS is not installed on this node" state, not a raw error. Same end result as today (can't manage a node without ANAS) with a clearer, distinguishable signal.
...
- **Uninstall:** restore pristine `AnyEvent.pm`, remove `/usr/share/anas/perl/AnasProxy.pm`, restart pveproxy.
