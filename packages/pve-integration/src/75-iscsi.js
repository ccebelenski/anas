...
(function () {
    ...
    // The UNIFIED restore dialog (68-backup.js) restores a new LUN onto the
                                ...
                                handler: function (btn) {
                                    ...
                                    ANAS.backup.openRestoreDialog(view, node, {
                                        lun: {
                                            targetIqn: iqn,
                                            index: rec.get('index'),
                                            serial: (rec.get('serial') == null) ? '' : ('' + rec.get('serial')),
                                            name: '' + (rec.get('name') || ''),
                                        },
                                        // Refresh the LUNs window and the view when
                                        // the job lands (or falls) — the restore
                                        // dialog lives in the backup side and the
                                        // iscsi screen is still here to repaint.
                                        onDone: function () {
                                            try {
                                                loadLuns(w, node, iqn);
                                            } catch (e) {
